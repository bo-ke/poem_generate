...
def gen_acrostic(opt, model, start_words, ix2word, word2ix, use_gpu, prefix_words=None):
    """
    生成藏头诗
    start_words : u'深度学习'
    生成：
...
def gen(opt):
    ...
    result = gen_poetry(opt, model, opt.start_words, ix2word, word2ix, opt.use_gpu, opt.prefix_words)
    # print(''.join(result))
    return "".join(result)
